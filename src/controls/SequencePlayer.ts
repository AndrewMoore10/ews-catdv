module controls
{
    ...
    class ItemPlayer
    {
        ...
        constructor(sequenceItem: SeqItemInfo, timelineOffset: number, playerControl: VideoPlayer)
        {
    ...
    export class SequencePlayer extends Control implements VideoPlayer
    {
        ...
        private sequenceIsPlaying: boolean;
        ...
        public openSequence(sequenceItems: SeqItemInfo[], useQuickTime: boolean, readyCallback: () => void)
        {
        ...
        public updateSequenceItems(sequenceItems: SeqItemInfo[])
        {
        ...
        private resizePlayer()
        {
            ...
            Console.debug("resizePlayer() - width: " + width + ", height:" + height);
        }

        private itemPlayer_PlayheadMoved(itemPlayerIndex: number, time: number, itemIsPlaying: boolean)
        {
                ...
                if (!itemPlayer.isLocked)
                {
                    ...
                    else if ((itemPlayer.clipOut - time) < 0.05)
                    {
                        ...
                        if (itemPlayerIndex < (this.itemPlayers.length - 1))
                        {
                            ...
                            nextItemPlayer.videoPlayer.setCurrentTime(nextItemPlayer.clipIn, this.sequenceIsPlaying ? PlayingState.Playing : PlayingState.Stopped);
