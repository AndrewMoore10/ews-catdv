module ui.panels
{
    ...
    import CachedServerDataSource = controls.CachedServerDataSource;
    ...
    import SearchFormManager = logic.SearchFormManager;
    import ServerCommandManager = logic.ServerCommandManager;
    import DetailPanelField = logic.DetailPanelField;
    import TextField = logic.TextField;
    import ComboBoxField = logic.ComboBoxField;
    import CheckBoxField = logic.CheckBoxField;
    import MultiCheckboxField = logic.MultiCheckboxField;
    ...
    var operatorLookup: { [valueType: string]: OperatorInfo[] } = {
        "text": [
            ...
            { name: "contains word", op: "hasWord", param_type: "text", logicalNOT: false }, // // word word "a phrase"           
        ...
        "multi": [
            ...
            { name: "contains one of", op: "hasOneOf", param_type: "text", logicalNOT: false }, // string,string,...
            { name: "does not contain any of", op: "hasOneOf", param_type: "text", logicalNOT: true }, // string,string,...           
            { name: "includes", op: "includes", param_type: "text", logicalNOT: false }, // word\nword (one or more multigrouping terms)            
    ...
    class QueryTermEditor extends Panel
    {
        ...
        private queryBuilder: QueryBuilderPanel;
        ...
        private fieldDataSource = new SimpleArrayDataSource<ListItem>();
        ...
        public setQueryTerm(queryTerm: QueryTerm)
        {
            ...
            var opertorInfo = this.operators.find((opertor) => opertor.op == queryTerm.op && opertor.logicalNOT == queryTerm.logicalNOT) || this.operators[0];
            ...
            if (operator)
            {
                switch (operator.param_type)
                {
                    case "text":
                        ...
                        {
                            var dataSource = new CachedServerDataSource<ListItem>((callback: (ListItemArray) => void) =>
                            {
                                FieldSettingsManager.getUniqueFieldValues(this.selectedFieldDef, (values: string[]) =>
                                {
    ...
    export class QueryBuilderPanel extends Panel
    {
    ...
    export class SimpleSearchForm extends Panel
    {
        private searchFormFields: DetailPanelField[] = null;
        ...
        public load(callback: (numFields: number) => void)
        {
            if (this.searchFormFields == null)
            {
                SearchFormManager.getSearchForm((searchForm: FormDefinition) => 
                {
                    this.searchFormFields = [];
                    if (searchForm && searchForm.fields)
                    {
                        var $searchFormTable = $("<table id='tblSearchForm' class='searchForm'></table>").appendTo(this.$element);

                        searchForm.fields.forEach((field, f) =>
                        {
                            ...
                            if (fieldDef)
                            {
                                ...
                                var searchField: DetailPanelField;
                                ...
                                this.searchFormFields.push(searchField);
                            }
                        });
                    }
        ...
        public clearQueryTerms()
        {
            this.setQueryTerms([]);
        }

        public setQueryTerms(queryTerms: QueryTerm[])
        {
            if (this.searchFormFields)
            {
                var t = 0;
                this.searchFormFields.forEach((searchFormField) =>
                {
            ...
            if (this.searchFormFields)
            {
                this.searchFormFields.forEach((searchFormField) =>
                {
                    ...
                    if (value)
                    {
                        ...
                        {
                            var values = (<Array<string>>value).join(",");
