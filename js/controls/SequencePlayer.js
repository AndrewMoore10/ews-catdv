...
(function (controls) {
    ...
    var SequencePlayer = (function (_super) {
        ...
        SequencePlayer.prototype.resizePlayer = function () {
            ...
            controls.Console.debug("resizePlayer() - width: " + width + ", height:" + height);
        };
        SequencePlayer.prototype.itemPlayer_PlayheadMoved = function (itemPlayerIndex, time, itemIsPlaying) {
            var itemPlayer = this.itemPlayers[itemPlayerIndex];
            if (itemPlayerIndex == this.currentItemPlayerIndex) {
                //                Console.debug("seq.onPlayheadMoved() itemPlayer[" + itemPlayerIndex + "].isLocked:" + itemPlayer.isLocked + ", itemIsPlaying:" + itemIsPlaying + ", sequenceIsPlaying=" + this.sequenceIsPlaying);
                if (!itemPlayer.isLocked) {
                    itemPlayer.isLocked = true;
                    if (itemIsPlaying && ((itemPlayer.clipIn - time) > 0.05)) {
                        itemPlayer.videoPlayer.setCurrentTime(itemPlayer.clipIn, controls.PlayingState.Playing);
                    }
                        ...
                        if (itemPlayerIndex < (this.itemPlayers.length - 1)) {
                            ...
                            nextItemPlayer.videoPlayer.setCurrentTime(nextItemPlayer.clipIn, this.sequenceIsPlaying ? controls.PlayingState.Playing : controls.PlayingState.Stopped);
                            nextItemPlayer.isLocked = false;
                            itemPlayer.hidePlayer();
                        }
                        else {
                            // end of last item in sequence - stop playing
                            this.sequenceIsPlaying = false;
                        }
                    }
                    this.currentTime = Math.max(itemPlayer.timelineOffset + (time - itemPlayer.clipIn), 0);
                    this.playheadMovedListeners.notifyListeners({ playerTimeSeconds: this.currentTime, isPlaying: this.sequenceIsPlaying });
                    itemPlayer.isLocked = false;
                }
            }
            else if (itemIsPlaying) {
                itemPlayer.videoPlayer.stop();
            }
        };
        ...
        SequencePlayer.prototype.togglePlay = function () {
            this.sequenceIsPlaying = !this.sequenceIsPlaying;
            if ((this.currentItemPlayerIndex >= 0) && (this.currentItemPlayerIndex < this.itemPlayers.length)) {
                if (this.sequenceIsPlaying) {
                    this.itemPlayers[this.currentItemPlayerIndex].videoPlayer.play();
                }
                else {
        ...
        SequencePlayer.prototype.setCurrentTime = function (time, stateAfterSeek) {
            if (stateAfterSeek === void 0) { stateAfterSeek = controls.PlayingState.Stopped; }
            ...
            if (targetItemPlayerIndex != this.currentItemPlayerIndex) {
                ...
                targetItemPlayer.videoPlayer.setCurrentTime(targetItemPlayer.clipIn + (time - targetItemPlayer.timelineOffset), controls.PlayingState.Stopped);
