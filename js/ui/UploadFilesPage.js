...
(function (ui) {
    ...
    var UploadFormManager = logic.UploadFormManager;
    ...
    var UploadFilesPage = (function () {
        function UploadFilesPage() {
            var _this = this;
            this.lblValidationError = new Label("lblValidationError");
            this.btnUpload = new Button("btnUpload");
            this.btnClose = new Button("btnClose");
            this.filesToUpload = [];
            this.allDone = false;
            this.uploadFormFields = [];
            ...
            this.btnUpload.onClick(function (e) {
                ...
                else if (_this.validateForm()) {
                    $("#btnAdd").prop("disabled", true);
                    $("#fileBrowser").prop("disabled", true);
                    $("#btnUpload").prop("disabled", true);
                    $("#btnClose").prop("disabled", true);
                    _this.uploadFile(0, _this.getUploadMetadata());
                }
            });
            this.btnClose.onClick(function (e) {
                self.close();
            });
            UploadFormManager.getUploadForm(function (uploadForm) {
                var $uploadFormTable = $("#tblUploadForm");
                if (uploadForm.fields) {
                    uploadForm.fields.forEach(function (field, f) {
                        if (field.fieldDefinition) {
                            ...
                            _this.uploadFormFields.push(fieldBinding);
                        }
                    });
                }
            });
        }
        UploadFilesPage.prototype.validateForm = function () {
            var valid = true;
            this.uploadFormFields.forEach(function (uploadFormField) {
        ...
        UploadFilesPage.prototype.getUploadMetadata = function () {
            var clipMetadata = {};
            this.uploadFormFields.forEach(function (uploadFormField) {
        ...
        UploadFilesPage.prototype.addFiles = function (files) {
            for (var i = 0; i < files.length; i++) {
                ...
                $("#tblFileList").append($("<tr>" + "<td width='50%'>" + HtmlUtil.escapeHtml(file.name) + "</td>" + "<td width='25%'>" + this.bytesToSize(file.size, 1) + "</td>"
        ...
        UploadFilesPage.prototype.uploadFile = function (fileIndex, uploadMetadata) {
            var _this = this;
            if ((fileIndex >= 0) && (fileIndex < this.filesToUpload.length)) {
                this.doFileUpload(this.filesToUpload[fileIndex], uploadMetadata, function (success) {
                    if (success) {
                        _this.uploadFile(fileIndex + 1, uploadMetadata);
                    }
                    else {
                        $("#btnClose").text("Close");
        ...
        UploadFilesPage.prototype.doFileUpload = function (fileInfo, uploadMetadata, callback) {
            var fileId = fileInfo.id;
            var progressDiv = $("#" + fileId + "_progress");
            var statusDiv = $(".progress-bar", $(progressDiv));
            $(progressDiv).scrollintoview();
            var file = fileInfo.file;
            $catdv.initiateUpload(file.name, file.size, uploadMetadata, function (reply) {
