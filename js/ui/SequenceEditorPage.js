...
(function (ui) {
    ...
    var SequenceEditorPage = (function () {
        function SequenceEditorPage() {
            ...
            this.clipsPanel = new ClipsPanel("clipListPanel", null, "ui.SequenceEditorPage");
            this.viewControls = new ViewControls("viewControls", this.clipsPanel, null);
            this.navigatorPanel = new NavigatorPanel("navigatorPanel", this.clipsPanel);
            $catdv.getClip(this.sequenceId, function (clip) {
                _this.setClip(clip);
            });
            this.clipsPanel.onQueryChanged(function (clipQuery) {
                _this.setListTitle(clipQuery.title);
            });
            this.navigatorPanel.onNavigationChanged(function (evt) {
            ...
            this.playerControls.onSetMarkIn(function (evt) {
                if (_this.selectedItem) {
                    var movieTime = _this.playerControls.getCurrentTime();
                    _this.selectedItem.clipIn = movieTime;
                    ...
                    _this.sequenceInOutsChanged(movieTime);
                }
            });
            this.playerControls.onSetMarkOut(function (evt) {
                if (_this.selectedItem) {
                    var movieTime = _this.playerControls.getCurrentTime();
                    _this.selectedItem.clipOut = movieTime;
        ...
        SequenceEditorPage.prototype.setListTitle = function (listTitle) {
            var listTitle = listTitle || "Clips";
            var colonIndex = listTitle.indexOf(":");
            if (colonIndex != -1) {
                this.lblListTitle.$element.html(listTitle.substr(0, colonIndex)
                    + "<span>" + HtmlUtil.escapeHtml(listTitle.substr(colonIndex + 1)) + "</span>");
            }
            else {
                this.lblListTitle.setText(listTitle);
            }
        };
        SequenceEditorPage.prototype.saveSequence = function () {
            ...
            TimecodeUtil.simplify(this.sequence["in"]),
                ...
                this.sequence.seqItems.forEach(function (seqItem) {
        ...
        SequenceEditorPage.prototype.sequenceInOutsChanged = function (newPlayheadTime) {
            this.recalculateSequenceInOuts();
            this.clipInfoPanel.updateUI();
            this.timelinePanel.sequenceInOutsChanged();
            this.timelinePanel.setPlayheadTime(newPlayheadTime);
    ...
    var ClipInfoPanel = (function (_super) {
        ...
        ClipInfoPanel.prototype.setSelectedItem = function (item) {
            this.item = item;
            if (item) {
                this.infoTabs.selectTab("clipTab");
            }
            else {
                this.infoTabs.selectTab("seqTab");
            }
            this.updateUI();
        };
    ...
    var Playhead = (function (_super) {
        ...
        function Playhead(element, minPosition, maxPosition) {
            _super.call(this, element, Direction.Horizontal, minPosition, maxPosition);
    ...
    var TimelinePanel = (function (_super) {
        ...
        function TimelinePanel(element) {
            ...
            trackPanel.onItemsReorders(function (items) { if (_this.reorderedHandler)
        ...
        TimelinePanel.prototype.sequenceInOutsChanged = function () {
            ...
            if (this.selectedItem)
                this.selectIndividualItem(this.selectedItem);
        };
    ...
    var TrackItem = (function (_super) {
        ...
        function TrackItem(element, seqItem, min_x, max_x) {
            _super.call(this, element, Direction.Horizontal, min_x, max_x);
    ...
    var TrackPanel = (function (_super) {
        ...
        TrackPanel.prototype.selectItem = function (seqItem) {
            var trackItem;
            this.trackItems.forEach(function (ti) { if (ti.seqItem == seqItem)
                trackItem = ti; });
            if (trackItem) {
    ...
    var ThumbnailLookup = (function () {
        ...
        ThumbnailLookup.getThumbnail = function (mediaID, time, callback) {
            ...
            if (thumbnails) {
                ThumbnailLookup.findThumbnailAtTime(thumbnails, time, callback);
            }
            else {
        ...
        ThumbnailLookup.findThumbnailAtTime = function (thumbnails, time, callback) {
            var thumbnailID = null;
            for (var i = 0; i < thumbnails.length; i++) {
                if (thumbnails[i].time >= time) {
                    thumbnailID = thumbnails[i].ID;
                    break;
                }
            }
            callback(thumbnailID != null ? thumbnailID : (thumbnails.length > 0 ? thumbnails[0].ID : null));
        };
